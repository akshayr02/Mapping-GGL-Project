...
@njit
def integrand_gammat_helper(ell, pkappas, ell_values, J2):
    """
    Helper function to compute the integrand for gammat.
...
class gammat:
    """
    Class for calculating gammat for weak gravitational lensing
    from power spectrum. 
    ...
    def precompute_pkappa(self, ell_values, tomo1, tomo2):
        ...
        pkappa_grid = self.limber.powerspectrum_projected_kappa_delta(ell_values, tomo1, tomo2)
    ...
    def integrand_gammat(self, ell, theta, tomo1, tomo2):
        
        combi=f"{tomo1}-{tomo2}"
        
        j2 = jv(2, ell * theta)
        
        return integrand_gammat_helper(ell, self.pkappas[combi], self.ell_values, j2)


    def gammat(self, theta, nell, tomo1, tomo2):
        """
        ...
        integrand_values = [self.integrand_gammat(ell, theta, tomo1, tomo2) for ell in ell_values]
